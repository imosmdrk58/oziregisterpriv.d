...
const { green , red } = require("../../../../../a-emoji.json")
module.exports = {
  conf: {
    aliases: ["sicil"],
    name: "sicil",
  ...
  run: async (client, message, args, embed) => {
    ...
    data = data.map((x) => `#${x.id} ${x.active ? green : red} **[${x.type}]** ${moment(x.date).format("LLL")} tarihinde, <@${x.staff}> tarafından, \`${x.reason}\` nedeniyle, ${x.type.toLowerCase().replace("-", " ")} cezası almış.`).join("\n");
